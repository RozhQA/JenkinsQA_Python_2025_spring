...
class SystemInformationPage(ManageJenkinsPage):
    ...
    def get_active_tab_number(self) -> int:
        tab = self.find_element(By.XPATH, "//div[@class='jenkins-tab-pane'][@style='display: block;']/h2")
    ...
    def get_table_content(self) -> list[list[str]]:
        ...
        table = self.find_element(By.XPATH, f"//div[@class='jenkins-tab-pane'][{tab_number}]/table/tbody")
    ...
    def click_show_all_values_button(self):
        tab_number = self.get_active_tab_number()
        self.click_on((By.XPATH, f"(//button[contains(normalize-space(text()), 'Show values')])[{tab_number}]"))

    def click_hide_all_values_button(self):
        tab_number = self.get_active_tab_number()
        self.click_on((By.XPATH, f"(//button[contains(normalize-space(text()), 'Hide values')])[{tab_number}]"))
